...
def read_standup():
    ...
    lines = []
    for filename in ["vv-standup.json", "caca-standup.json", "rasp-standup.json", "mimi-standup.json", "potato-standup.json"]:
...
def get_tagline():
    weekday = datetime.now(timezone(timedelta(hours=8))).weekday()
    return TAGLINES[weekday]
